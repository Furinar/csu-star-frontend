...
export default function TeacherSlider() {
  ...
  return (
      <>
        <div
            className="teacher-slider-container grid h-auto min-h-[220px] w-full grid-cols-[2fr_1fr] md:h-[400px] md:min-h-0 md:grid-cols-[7fr_3fr] md:py-10"
            style={{
              background:
        ...
        >
          <div className="teacher-slider flex items-center justify-center">
            <div
                className="box h-auto min-h-[220px] w-full md:h-[292px] md:min-h-0"
            ...
            >
              ...
              <button type="button" className="slider-switch slider-switch-prev" onClick={() => shiftIndex(-1)}
                      aria-label="查看上一位教师">
                <span></span>
              </button>
              <button type="button" className="slider-switch slider-switch-next" onClick={() => shiftIndex(1)}
                      aria-label="查看下一位教师">
                <span></span>
              </button>
            </div>
          ...
          <div className="teacher-introduce-container pl-1 pr-2 py-1.5 md:px-0 md:py-3">
            <div
                className="inline-flex h-auto min-h-[220px] flex-col justify-center gap-1.5 py-0.5 md:h-full md:min-h-0 md:items-end md:justify-between md:gap-2 md:py-0">
              <h1 className="mt-0 hidden text-lg font-bold leading-none md:mt-0 md:block md:self-end md:text-right md:text-[28px]">
              ...
              {hasError && !isLoading && teachers.length === 0 ? (
                  ...
                  <div className="flex h-full items-center justify-end gap-1.5 md:w-full md:gap-5">
                    ...
                    <div
                        className="detail flex min-w-0 flex-1 flex-col items-end gap-0.5 text-right md:max-w-[220px] md:gap-2">
                      {currentTeacherPath ? (
                          <Link href={currentTeacherPath}
                                className="name hero-gradient-text line-clamp-2 text-right text-base font-bold leading-tight md:text-2xl">
                            {currentTeacher.name}
                          </Link>
                      ) : (
              ...
              <div className="teacher-links mb-0 hidden justify-end md:flex md:w-full">
                {currentTeacherPath ? (
                    <Link
                    ...
                    <div
                        className="button button--flex flex w-full cursor-not-allowed justify-center px-2.5 py-1.5 text-xs shadow-lg opacity-60 md:px-5 md:py-2.5 md:text-base">
                      查看当前教师
