...
export default function TeacherGlobalEvaluationModal({
  ...
  return (
    <DetailComposerModal
      ...
      badge="教师评价"
      title={
        selectedTeacher ? `为 ${selectedTeacher.name} 写一条评价` : "先选择教师"
      }
      description={
        selectedTeacher
      ...
      {!selectedTeacher ? (
        <div className="mx-auto max-w-2xl px-1 sm:px-2">
                ...
                <div className="h-full">
                  {options.map((teacher) => (
                    ...
                    >
                      ...
                      <span className="text-xs font-medium text-[var(--first-color)] opacity-80 mt-0.5">
                        选择
                      </span>
                    </button>
                  ))}
          ...
          {!isSearching && query.trim() && options.length === 0 ? (
            <div className="mt-4 text-sm text-slate-400">未找到相关教师</div>
          ) : null}
        </div>
      ) : (
        <div className="flex flex-col gap-4">
          <div className="mx-auto flex w-full max-w-4xl flex-col gap-3 rounded-3xl border border-slate-100 bg-slate-50/70 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            <div className="min-w-0">
              <div className="text-[11px] font-semibold uppercase tracking-wider text-rose-500/80 mb-0.5">
                SELECTED TEACHER
              </div>
