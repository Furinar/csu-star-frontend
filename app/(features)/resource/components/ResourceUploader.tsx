...
import {AdvancedInput, AdvancedSelect, AdvancedTextarea, UnderlineInput,} from "./AdvancedFormControls";
...
export default function ResourceUploader({
  ...
  return (
      ...
      >
        <div className="mb-6">
          <h2 className="text-xl font-bold text-black sm:text-2xl mb-4">上传资源</h2>
          <div className="relative">
            <div className="text-[11px] font-semibold uppercase tracking-wider text-sky-500/80 mb-1">
              BINDING COURSE
            </div>
            {selectedCourse ? (
                ...
                <div className="w-[60%] sm:max-w-xs">
                  <UnderlineInput
                      label={
                        <>
                          搜索课程名称进行绑定 <span className="text-red-500">*</span>
                        </>
                      }
                      type="text"
                      value={courseQuery}
        ...
        {uploadedResourceId ? (
            ...
            <div className="space-y-6">
                ...
                <div>
                  ...
                  {!descExpanded && (
                      <button
                          ...
                          className="flex w-full items-center gap-1.5 text-sm text-slate-400 hover:text-slate-600 transition-colors md:hidden"
                      >
                        <i className="uil uil-plus text-base"/>
                        添加资源简介
                      </button>
                  )}
                  <div className={descExpanded ? "" : "hidden md:block"}>
                    <AdvancedTextarea
                        label="资源简介 (可选)"
                        value={description}
              ...
              <div
                  className={
                    files.length > 0
                        ? "space-y-6 lg:space-y-0 lg:grid lg:grid-cols-2 lg:gap-6"
                        : ""
                  }
              >
                <div className="flex flex-col h-full min-h-0">
                  ...
                  <button
                      ...
                      disabled={isUploading}
                      className={`md:hidden w-full flex items-center justify-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2.5 text-sm font-medium text-emerald-700 transition-colors hover:bg-emerald-100 mb-3 ${isUploading ? "opacity-50 cursor-not-allowed" : ""}`}
                  >
                    <i className="uil uil-cloud-upload text-lg"/>
                    上传文件 <span className="text-red-500">*</span>
                  </button>
                  {/* 桌面端：虚线拖拽框 */}
                  <label className="hidden md:block shrink-0 text-sm font-medium text-black mb-2">
                    上传文件 <span className="text-red-500">*</span>
                  </label>
                  <div
                      ...
                      onDrop={handleDrop}
                      onClick={() => !isUploading && fileInputRef.current?.click()}
                      className={`hidden md:flex flex-1 flex-col justify-center cursor-pointer rounded-xl border-2 border-dashed p-6 text-center transition-colors sm:p-8
                ...
                {files.length > 0 && (
                    <div className="relative w-full">
                      <div className="flex flex-col h-full lg:absolute lg:inset-0">
                        <h4 className="shrink-0 mb-3 text-sm font-medium text-black">
                          待上传文件 ({files.length})
                        </h4>
                        <div className="flex-1 overflow-y-auto pr-1 sm:pr-2 max-h-48 lg:max-h-none space-y-2">
                          {files.map((f) => (
                              <div
                                  key={f.local_id}
                              ...
                              >
                                ...
                                <div className="relative z-10 flex items-center gap-2 sm:justify-between">
                                  <div className="flex flex-1 min-w-0 items-center gap-2 sm:gap-3">
                                    <i className="uil uil-file-alt shrink-0 text-xl text-emerald-400"/>
                                    ...
                                    {editingFileId !== f.local_id && (
                                        <div
                                            className="hidden sm:block shrink-0 ml-auto pr-2 text-xs text-second sm:w-16 sm:text-right">
              ...
              <div className="flex flex-row items-center justify-end pt-2 sm:pt-4 gap-3 sm:gap-4">
                {files.length > 0 && (
                    <span className="text-sm font-medium text-slate-400">
                {(
                    files.reduce((a, b) => a + b.file.size, 0) /
                    1024 /
                    1024
                ).toFixed(2)}{" "}
                      MB / 300 MB
