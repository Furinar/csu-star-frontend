...
export default function CourseGlobalEvaluationModal({ isOpen, onClose }: CourseGlobalEvaluationModalProps) {
  ...
  const handleSubmit = async (payload: Record<string, unknown>) => {
    ...
    try {
      await createCourseEvaluation(selectedCourse.id, payload as unknown as CourseEvaluationInput);
  ...
  return (
    <DetailComposerModal
      ...
      accent="course"
      badge="课程评价"
      title={selectedCourse ? `为 ${selectedCourse.name} 写一条评价` : "写一条课程评价"}
      description={selectedCourse ? "您的评价将帮助更多同学" : "搜索并选择你想评价的课程"}
    >
      {!selectedCourse ? (
        <div className="relative mt-4 h-64">
          <AdvancedInput
            label={<>搜索课程 <span className="text-red-500">*</span></>}
            value={query}
            ...
            <div className="absolute z-10 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg max-h-48 overflow-y-auto">
              {options.map((course) => (
                ...
                >
                  ...
                  {course.course_type && <div className="text-xs text-slate-400 mt-0.5">{course.course_type}</div>}
                </div>
              ))}
            </div>
          )}
          {!isSearching && query.trim() && options.length === 0 && (
            <div className="text-sm text-slate-500 mt-2 px-2">未找到相关课程</div>
          )}
        </div>
      ) : (
        <div className="mt-6 flex flex-col gap-6">
          <div className="flex items-center justify-between bg-sky-50 px-4 py-3 rounded-xl border border-sky-100">
            <span className="font-medium text-sky-800">已选课程：{selectedCourse.name}</span>
            <button
              type="button"
