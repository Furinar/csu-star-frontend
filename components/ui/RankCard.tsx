...
export default function RankCard({title, data, onItemClick, onViewAll}: RankCardProps) {
  return (
      <div
          className="flex min-w-[280px] flex-1 flex-col rounded-3xl border border-white/40 bg-white/40 p-6 shadow-[0_8px_32px_0_rgba(31,38,135,0.07)] backdrop-blur-xl transition-all duration-300 hover:shadow-[0_8px_32px_0_rgba(31,38,135,0.15)]">
        ...
        <div className="flex flex-col gap-3">
          {data.slice(0, 10).map((item, index) => {
            ...
            if (index === 0)
              medalIcon = (
                  <span
                      className="text-lg font-black text-transparent bg-clip-text bg-gradient-to-br from-yellow-300 to-amber-600">
                1
              </span>
              );
            else if (index === 1)
              medalIcon = (
                  <span
                      className="text-lg font-black text-transparent bg-clip-text bg-gradient-to-br from-slate-300 to-slate-500">
                2
              </span>
              );
            else if (index === 2)
              medalIcon = (
                  <span
                      className="text-lg font-black text-transparent bg-clip-text bg-gradient-to-br from-orange-400 to-red-600">
                3
              </span>
              );
            else
