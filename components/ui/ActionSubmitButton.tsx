...
const ActionSubmitButton = ({ 
  ...
  const letters = defaultText.split("");
        ...
        <div className="state state--sent">
          ...
          <p>
            {sentLetters.map((char, index) => (
              <span key={index} style={{ "--i": letters.length + index } as React.CSSProperties}>{char === " " ? "\u00A0" : char}</span>
            ))}
          </p>
        </div>
      </button>
...
const StyledWrapper = styled.div`
  ...
  .button:focus p span, .button.is-sent p span {
    opacity: 1;
  ...
  .button:focus .state--default svg, .button.is-sent .state--default svg {
    animation: takeOff 0.8s linear forwards;
  }
  .button:focus .state--default .icon, .button.is-sent .state--default .icon {
  ...
  .button:focus .state--default .icon:before, .button.is-sent .state--default .icon:before {
  ...
  .button:focus .state--default, .button.is-sent .state--default {
    position: absolute;
    pointer-events: none;
  }
  .button:focus .state--sent, .button.is-sent .state--sent {
    display: flex;
  }
  .button:focus .state--sent span, .button.is-sent .state--sent span {
    opacity: 0;
    animation: slideDown 0.8s ease forwards calc(var(--i) * 0.2s);
  }
  .button:focus .state--sent .icon svg, .button.is-sent .state--sent .icon svg {
    opacity: 0;
