...
import styles from "./ActionSubmitButton.module.css";
...
const ActionSubmitButton = ({
  ...
  return (
    <div className={styles.wrapper}>
      <button
        className={`${styles.button} ${isSent ? styles.isSent : ""}`}
        disabled={disabled}
        type={type}
        {...props}
      >
          ...
          <p>
            {sentLetters.map((char, index) => (
              <span
                key={index}
                style={
                  { "--i": letters.length + index } as React.CSSProperties
                }
              >
                {char === " " ? "\u00A0" : char}
              </span>
