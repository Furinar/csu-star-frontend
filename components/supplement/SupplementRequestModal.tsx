...
export default function SupplementRequestModal({
  ...
  const buildPayload = (): CreateSupplementRequestInput | null => {
    ...
    if (currentType === "teacher") {
      ...
      const relatedCourseName = form.related_course_name.trim();
      const departmentId = Number(form.department_id);

      if (!teacherName || !form.department_id || !Number.isFinite(departmentId) || !relatedCourseName) {
        feedback.warning({
          title: "教师补录信息不完整",
          description: "请填写老师姓名、学院和相关课程。",
        });
        return null;
      }
      ...
      return {
        ...
        related_course_name: relatedCourseName,
